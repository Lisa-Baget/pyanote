...
CONTROLEUR_STANDARD = {
    ...
    'temps_ticks' : 0, ## nombre de ticks écoulés depuis le début de la chanson
    'temps_micros' : 0, ## nombre de microsecondes depuis le début de la chanson
    ## Remarque 1: il faut garder les deux parce qu'il peut y avoir eu plusieurs changements de tempo
    ## Remarque 2: vrai temps = temps_micros + temps execution programme qu'on ne compte pas 
    'vitesse' : 1,
    'fin' : False, ## le mettre à True pour finir la lecture
    'pause' : False, ## le mettre à True pour mettre la lecture en pause
    'seconde_transmise' : 0 ## mecanisme de transmission du temps vers l'interface 
}
...
def creer_controleur(nom_fichier, sortie_midi, widget=False, karaoke=True):
    ...
    initialiser_controleur(controleur) ## rajoute toutes les valeurs initiales
    controleur['midi'], controleur['widget'] = sortie_midi, widget
    controleur["nom"] = nom_fichier.split('/')[-1] # decoupe le chemin, le dernier c'est le nom du fichier
    controleur["pistes actives"] = [True] * controleur['nb_pistes']
...
def initialiser_controleur(controleur):
    controleur.update(CONTROLEUR_STANDARD) ## on lui rajoute toutes les clés/valeurs du controleur standard
...
def jouer_album(controleur):
    ...
    while (not controleur['fin']):
        if controleur['pause']:
            ...
            evenement = album[controleur["index_chanson"]][controleur["index_evenement"]]
...
def maj_evenement(controleur):
    album = controleur['chansons']
    if controleur["index_evenement"] + 1 == len(album[controleur["index_chanson"]]): # on a traité le dernier evenement de la piste
        if controleur["index_chanson"] + 1 == len(album): # on a traité la dernière piste
            controleur["fin"] = True ## faudrait peut etre prevoir une lecture en boucle
        else:
            controleur["index_chanson"] += 1 # piste suivante
            controleur["index_evenement"] = 0
            time.sleep(1) ## on a fini une chanson, on attend un peu
